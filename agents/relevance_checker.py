...
class RelevanceChecker:
    ...
    def check(self, question: str, retriever, k=3) -> str:
        ...
        if not top_docs:
            logger.debug("No documents returned from the retriever.invoke(). Classifying as NO_MATCH.")
            return "NO_MATCH"
        ...
        **Respond ONLY with one of the following labels: CAN_ANSWER, PARTIAL, NO_MATCH**
        ...
        try:
            llm_response = response['choices'][0]['messages']['content'].strip().upper()
        ...
        if llm_response not in valid_labels:
            logger.debug("LLM didnot respond with a valid label. Forcing 'NO_MATCH'.")
